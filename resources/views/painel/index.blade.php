                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="quantidade_por_folha" name="quantidade_por_folha" required>
                                        ...
                                        <option value="2">2 cartelas por folha (iguais)</option>
                                        <option value="3">3 cartelas por folha (iguais)</option>
                                        <option value="4">4 cartelas por folha (iguais)</option>
                                        <option value="5">5 cartelas por folha (iguais)</option>
                                        <option value="6">6 cartelas por folha (iguais)</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label for="cartelas_por_arquivo" class="form-label">Cartelas por Arquivo PDF</label>
                                    <input type="number" class="form-control" id="cartelas_por_arquivo" name="cartelas_por_arquivo" min="100" max="5000" value="1000" required>
                                </div>
                                <div class="mb-3">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                buttonGerarCartelasFesta.addEventListener('click', function () {
                    ...
                    const quantidade_por_folha = modal.querySelector('#quantidade_por_folha').value;
                    const cartelas_por_arquivo = modal.querySelector('#cartelas_por_arquivo').value;
                    const texto_lateral = modal.querySelector('#texto_lateral').value;

                    let formData = new FormData()
                    formData.append('festa_id', festaId);
                    formData.append('quantidade', quantidade);
                    formData.append('quantidade_por_folha', quantidade_por_folha);
                    formData.append('cartelas_por_arquivo', cartelas_por_arquivo);
                    formData.append('texto_lateral', texto_lateral);
